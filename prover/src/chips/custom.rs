    ...
    mod state {
            ...
            stwo_constraint_framework::Relation<F, EF> for StateLookupElements
        {
            fn combine(&self, values: &[F]) -> EF {
                <RawStateLookupElements as stwo_constraint_framework::Relation<F, EF>>::combine(&self.0, values)
            }

            fn get_name(&self) -> &str {
                <RawStateLookupElements as stwo_constraint_framework::Relation<F, EF>>::get_name(&self.0)
            }

            fn get_size(&self) -> usize {
                <RawStateLookupElements as stwo_constraint_framework::Relation<F, EF>>::get_size(&self.0)
            }
        }
    }
}
