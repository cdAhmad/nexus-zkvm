...
impl MachineChip for TimestampChip {
    fn fill_main_trace(
        ...
        let (c_reg1_ts_prev, ch1_minus) = decr_subtract_with_borrow(
            reg1_ts_cur.to_le_bytes(),
            reg1_ts_prev.to_le_bytes()
        );
        let (c_reg2_ts_prev, ch2_minus) = decr_subtract_with_borrow(
            reg2_ts_cur.to_le_bytes(),
            reg2_ts_prev.to_le_bytes()
        );
        let (c_reg3_ts_prev, ch3_minus) = decr_subtract_with_borrow(
            reg3_ts_cur.to_le_bytes(),
            reg3_ts_prev.to_le_bytes()
        );
...
mod test {
    ...
    fn test_timestamp_check_success() {
        ...
        let trace_steps = program_steps.chain(std::iter::repeat(None)).take(traces.num_rows());
