...
use stwo_constraint_framework::{ LogupTraceGenerator, EvalAtRow, Relation, RelationEntry };
...
impl MachineChip for RegisterMemCheckChip {
    ...
    fn fill_main_trace(
        ...
        let reg3_accessed: [BaseField; 1] = virtual_column::Reg3Accessed::read_from_traces_builder(
            traces,
            row_idx
        );
...
impl RegisterMemCheckChip {
    ...
    fn constrain_subtract_prev_reg<E: EvalAtRow>(
        ...
        eval.add_to_relation(RelationEntry::new(lookup_elements, numerator.into(), &tuple));
    }
...
fn fill_prev_values(
    ...
    let AccessResult { prev_timestamp, prev_value } = side_note.register_mem_check.access(
        reg_idx,
        reg_cur_ts,
        cur_value
    );
