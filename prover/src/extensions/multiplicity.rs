...
use crate::{
    chips::range_check::{
        range128::Range128LookupElements,
        range16::Range16LookupElements,
        range256::Range256LookupElements,
        range32::Range32LookupElements,
    },
...
impl<const LEN: usize, L: RegisteredLookupBound> FrameworkEval for MultiplicityEval<LEN, L> {
    ...
    fn evaluate<E: stwo_constraint_framework::EvalAtRow>(&self, mut eval: E) -> E {
        ...
        eval.add_to_relation(
            RelationEntry::new(lookup_elements, (-multiplicity).into(), &[checked_value])
        );
        eval.finalize_logup();
        eval
    }
...
impl<const LEN: usize, L: RegisteredLookupBound> BuiltInExtension
    for Multiplicity<LEN, L>
    where
...
impl<const LEN: usize, L> Multiplicity<LEN, L> {
    ...
    fn base_columns(side_note: &SideNote) -> Vec<BaseColumn>
        where SideNote: RangeCheckSideNoteGetter<LEN>
    {
        ...
        let multiplicities = BaseColumn::from_iter(
            range_check_side_note.multiplicity.into_iter().map(BaseField::from)
        );
        vec![multiplicities]
    }
}
