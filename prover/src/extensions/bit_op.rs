...
impl FrameworkEval for BitOpMultiplicityEval {
    ...
    fn evaluate<E: stwo_constraint_framework::EvalAtRow>(&self, mut eval: E) -> E {
        ...
        let preprocessed_columns: Vec<E::F> = PREPROCESSED_COL_IDS.iter()
...
impl BuiltInExtension for BitOpMultiplicity {
    ...
    fn generate_interaction_trace(
        ...
        let [answer_b, answer_c, answer_a_and, answer_a_or, answer_a_xor] = std::array::from_fn(
            |i| &preprocessed_columns[i]
        );
...
impl BitOpMultiplicity {
    fn preprocessed_base_columns() -> Vec<BaseColumn> {
        ...
        let column_and = BaseColumn::from_iter(
            range_iter.clone().map(|(b, c)| u32::from(b & c).into())
        );
        let column_or = BaseColumn::from_iter(
            range_iter.clone().map(|(b, c)| u32::from(b | c).into())
        );
        let column_xor = BaseColumn::from_iter(
            range_iter.clone().map(|(b, c)| u32::from(b ^ c).into())
        );
