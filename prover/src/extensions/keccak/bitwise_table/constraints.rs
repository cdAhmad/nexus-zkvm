...
> BitwiseTableEvalAtRow<'_, ELEM_BITS, EXPAND_BITS, E, R, B> {
    pub fn eval(mut self) -> E {
        ...
        let al = self.eval.get_preprocessed_column(
            BitwiseTable::new(ELEM_BITS, EXPAND_BITS, 0).id()
        );

        let bl = self.eval.get_preprocessed_column(
            BitwiseTable::new(ELEM_BITS, EXPAND_BITS, 1).id()
        );
        ...
        for i in 0..1 << (2 * EXPAND_BITS) {
            ...
            let c =
                ...
                E::F::from(
                    BaseField::from_u32_unchecked(
                        high <<
                            BitwiseTable::new(ELEM_BITS, EXPAND_BITS, B::C_COL_INDEX).limb_bits()
                    )
                );
