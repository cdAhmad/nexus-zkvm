...
impl<E: EvalAtRow> KeccakRoundEval<'_, E> {
    pub fn eval(mut self) -> E {
        ...
        for x in 0..5 {
            for y in 0..5 {
                b[y + ((2 * x + 3 * y) % 5) * 5] = self.rotate_left(
                    &a[x + y * 5],
                    ROTATIONS[x + y * 5]
                );
            }
        ...
        let is_padding = {
            ...
            let [is_padding, _next_is_padding] = self.eval.next_interaction_mask(
                ORIGINAL_TRACE_IDX,
                [0, 1]
            );
            ...
            self.eval.add_constraint(is_padding.clone() * (E::F::one() - is_padding.clone()));
    ...
    fn rotate_left(&mut self, a: &[E::F; LANE_SIZE], r: usize) -> [E::F; LANE_SIZE] {
        ...
        for i in 0i8..8 {
            ...
            self.eval.add_constraint(
                bits_low[low as usize].clone() +
                    bits_high[high as usize].clone() -
