...
use stwo::{
    prover::poly::circle::{ CircleEvaluation },
    prover::poly::BitReversedOrder,
...
impl FrameworkEval for MultiplicityEval8 {
    ...
    fn evaluate<E: stwo_constraint_framework::EvalAtRow>(&self, mut eval: E) -> E {
        ...
        eval.add_to_relation(
            RelationEntry::new(&self.lookup_elements, (-multiplicity).into(), &[checked_value])
        );
        eval.finalize_logup();
        eval
    }
...
impl Multiplicity8 {
    fn num_padding() -> usize {
        (1 << LOG_N_LANES).checked_sub(&8).expect("Code assumes SIMD lanes should be at least 8")
    }
